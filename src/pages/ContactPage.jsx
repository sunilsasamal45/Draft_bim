...
function SuccessPanel({ onBack }) {
  return (
    <div className="flex flex-col items-center justify-center gap-6 py-10 text-center">
      <CheckCircle2 size={52} className="text-brand" strokeWidth={1.5} />
      <div>
        <p className="text-xs font-extrabold uppercase tracking-[.18em] text-brand">Inquiry Received</p>
        <h2 className="mt-3 text-2xl font-extrabold tracking-[-.045em] text-ink">
          Thank you for contacting Draft BIM.
        </h2>
        <p className="mx-auto mt-4 max-w-sm text-sm leading-6 text-slate-600">
          Your project inquiry has been successfully received. Our team will review your requirements and get back to you shortly.
        </p>
      ...
      <button
        type="button"
        onClick={onBack}
        className="mt-2 text-sm font-extrabold text-brand underline underline-offset-4"
      >
...
export default function ContactPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setSubmitError(
        'Something went wrong while sending your inquiry. Please try again or contact us directly at Draftbim@gmail.com.'
      )
    }
  }
      ...
      <section className="relative py-20 lg:py-28">
        <Container>
          ...
          <div className="mt-16 grid gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
            ...
            {/* Right — trust points + form/success */}
                ...
                <AnimatePresence mode="wait">
                  {stage === 'success' ? (
                    <motion.div key="success" initial={reducedMotion ? false : { opacity: 0, rotateY: -180, scale: .94 }} animate={{ opacity: 1, rotateY: 0, scale: 1 }} exit={{ opacity: 0, rotateY: 180, scale: .94 }} transition={{ duration: .75, ease: [0.22, 1, 0.36, 1] }} className="rounded-2xl border border-white/80 bg-white/90 p-6 shadow-2xl shadow-ink/10 backdrop-blur sm:p-8">
                      <SuccessPanel onBack={() => setStage('form')} />
                    </motion.div>
                  ) : (
                    <motion.div key="form" initial={reducedMotion ? false : { opacity: 0, rotateY: -180, scale: .94 }} animate={{ opacity: 1, rotateY: 0, scale: 1 }} exit={{ opacity: 0, rotateY: 180, scale: .94 }} transition={{ duration: .75, ease: [0.22, 1, 0.36, 1] }} className="rounded-2xl border border-white/80 bg-white/85 p-6 shadow-2xl shadow-ink/10 backdrop-blur sm:p-8">
                      <form onSubmit={handleSubmit(onSubmit)} noValidate>
                        ...
                        {submitError && (
                          <p role="alert" className="mt-5 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-xs font-semibold leading-5 text-red-700">
                            {submitError}
                          </p>
                        )}
