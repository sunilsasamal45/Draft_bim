...
import { ArrowRight, Building2, CalendarDays, Clock3, LockKeyhole, Mail, MapPin, Phone, Send, ShieldCheck } from 'lucide-react'
...
function Field({ label, name, error, register, type = 'text', required = name !== 'message', options, placeholder, rows }) {
  ...
  return (
    <div className="relative">
      ...
      {options
        ? <select id={name} defaultValue="" className={`${inputClass} h-14 appearance-none`} {...registration}><option value="" disabled>Select a project type</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select>
        : rows
...
function CalendlyEmbed({ details, onBack }) {
  const params = new URLSearchParams({ hide_gdpr_survey: '1', name: details.name || '', email: details.email || '' })
  const meetingUrl = `https://calendly.com/draftbim/consultation?${params.toString()}`
  return (
    <div>
      <div className="flex items-start justify-between gap-5">
        <div>
          <p className="text-xs font-extrabold uppercase tracking-[.16em] text-brand">Schedule a meeting</p>
          <h2 className="mt-2 text-3xl font-extrabold tracking-[-.045em] text-ink">Book your consultation.</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">Select a time that works for you. You'll receive a confirmation directly.</p>
        </div>
        <CalendarDays className="mt-1 shrink-0 text-brand" size={26} />
      </div>
      <div className="mt-6 -mx-6 overflow-hidden border-y border-slate-200 bg-white sm:mx-0 sm:rounded-xl sm:border">
        <iframe title="Schedule a consultation with Draft BIM" src={meetingUrl} className="block h-[760px] min-w-[320px] w-full sm:h-[700px]" loading="lazy" />
      </div>
      <button type="button" onClick={onBack} className="mt-5 text-sm font-extrabold text-brand underline underline-offset-4">
...
export default function ContactPage() {
  ...
  const words = "Let's Discuss Your Project".split(' ')
      ...
      <section className="relative py-20 lg:py-28">
        <Container>
          ...
          <div className="mx-auto max-w-5xl text-center">
            ...
            <h1 className="mt-5 text-4xl font-extrabold leading-[1.08] tracking-[-.055em] text-ink sm:text-6xl">
              {words.map((word, index) => (
                <motion.span key={`${word}-${index}`} initial={reducedMotion ? false : { opacity: 0, y: 22 }} animate={reducedMotion ? {} : { opacity: 1, y: 0 }} transition={{ delay: .1 + index * .065, duration: .45 }} className="mr-[.24em] inline-block last:mr-0">
                  {word}
                </motion.span>
              ))}
            </h1>
            <motion.p initial={reducedMotion ? false : { opacity: 0, y: 16 }} animate={reducedMotion ? {} : { opacity: 1, y: 0 }} transition={{ delay: .4, duration: .5 }} className="mx-auto mt-6 max-w-3xl text-base leading-7 text-slate-600">
          ...
          <div className="mt-16 grid gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
            ...
            <motion.div initial={reducedMotion ? false : { opacity: 0, x: -26 }} whileInView={reducedMotion ? {} : { opacity: 1, x: 0 }} viewport={{ once: true, amount: .15 }} transition={{ duration: .65 }}>
              <div className="group relative overflow-hidden rounded-2xl border border-white/70 bg-white p-2 shadow-xl shadow-ink/10">
                <img loading="lazy" src={contactImage} alt="Draft BIM Koraput office and consultation" className="image-drift aspect-[4/3] w-full rounded-xl object-cover transition duration-700 group-hover:scale-105" />
            ...
            {/* Right — trust points + form/calendar */}
            <motion.div initial={reducedMotion ? false : { opacity: 0, x: 26 }} whileInView={reducedMotion ? {} : { opacity: 1, x: 0 }} viewport={{ once: true, amount: .12 }} transition={{ duration: .65 }}>
              <div className="grid gap-3 sm:grid-cols-3">
                {trustPoints.map(([Icon, title, copy], index) => (
                  <motion.div key={title} initial={reducedMotion ? false : { opacity: 0, y: 12 }} whileInView={reducedMotion ? {} : { opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * .08 }} className="rounded-xl border border-brand/10 bg-white/70 p-4 backdrop-blur">
                      ...
                      <form onSubmit={handleSubmit(onSubmit)} noValidate>
                        <div className="flex items-start justify-between gap-6">
                          <div>
                            ...
                            <h2 className="mt-2 text-3xl font-extrabold tracking-[-.045em] text-ink">Tell us about your project.</h2>
                          </div>
                        ...
                        <div className="mt-7 grid gap-5 sm:grid-cols-2">
                          ...
                          <Field label="Email Address" name="email" type="email" error={errors.email} register={(name, rules) => register(name, { ...rules, pattern: { value: /^\S+@\S+\.\S+$/, message: 'Enter a valid email address' } })} placeholder="you@example.com" />
                          <div className="sm:col-span-2">
                            <Field label="Project Type" name="projectType" error={errors.projectType} register={register} options={projectTypes} />
                          </div>
                          ...
                          <div className="sm:col-span-2">
                            <Field label="Message" name="message" error={errors.message} register={register} placeholder="Providing accurate details helps us understand your requirements clearly and offer the best possible design and consultancy solution." rows={5} />
                          </div>
                        </div>
                        <Button type="submit" disabled={isSubmitting} className="mt-7 w-full disabled:cursor-not-allowed disabled:opacity-60">
                          {isSubmitting
                        ...
                        <p className="mt-4 text-center text-xs leading-5 text-slate-500">
                          Your details are used only to respond to your enquiry and are kept confidential.
                        </p>
                      </form>
                    </motion.div>
                  )}
