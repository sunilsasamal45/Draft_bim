...
function CalendlyEmbed({ details, onBack }) {
  const params = new URLSearchParams({ hide_gdpr_survey: '1', name: details.firstName || '', email: details.email || '' })
  return (
    <div>
      <div className="flex items-start justify-between gap-5">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">Select a time that works for you.</p>
        </div>
      ...
      <div className="mt-6 -mx-6 overflow-hidden border-y border-slate-200 bg-white sm:mx-0 sm:rounded-xl sm:border">
        <iframe title="Schedule a consultation with Draft BIM" src={`https://calendly.com/draftbim/consultation?${params}`} className="block h-[760px] min-w-[320px] w-full sm:h-[700px]" loading="lazy" />
      </div>
      <button type="button" onClick={onBack} className="mt-5 text-sm font-extrabold text-brand underline underline-offset-4">Back to project details</button>
    </div>
  )
}
